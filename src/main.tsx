...
import { ChevronLeft, ChevronRight, FileImage, FolderOpen, HelpCircle, Image, Info, LoaderCircle, Moon, Plus, Settings2, Sun, Upload, X } from "lucide-react";
import "./styles.css";
...
type Settings = { format: Format; unit: Unit; width: number | null; height: number | null; ppi: number; resolutionVersion: number; resizeMode: ResizeMode; cropAnchor: CropAnchor; quality: Quality };
...
const baseSettings: Settings = { format: "jpeg", unit: "px", width: null, height: null, ppi: 72, resolutionVersion: 1, resizeMode: "contain", cropAnchor: "center", quality: "high" };
const ppiOptions = [
  { value: 72, label: "72 (Recommended)" },
  { value: 96, label: "96" },
  { value: 144, label: "144" },
  { value: 192, label: "192" },
  { value: 300, label: "300" }
...
function App() {
  ...
  useEffect(() => {
    if (!summary && !helpOpen) return;
    const closeDialog = (event: KeyboardEvent) => {
      if (event.key === "Escape") { setSummary(null); setHelpOpen(false); }
    };
  ...
  return <main className="studio">
    ...
    <aside className="inspector"><section className="inspector-body">
      <div className="inspector-heading"><Settings2 size={18}/><div><strong>Export settings</strong><span>Choose the output size and format.</span></div></div>
      <div className="format-row"><label>Format<select value={settings.format} onChange={(event) => update("format", event.target.value as Format)}>{formats.map((format) => <option key={format} value={format}>{formatLabel[format]}</option>)}</select></label><label>Units<select value={settings.unit} onChange={(event) => update("unit", event.target.value as Unit)}><option value="px">px</option><option value="in">in</option><option value="cm">cm</option><option value="mm">mm</option><option value="pt">pt</option></select></label></div>
      <div className="quality-resolution-row"><label>Image quality<select disabled={settings.format !== "jpeg" && settings.format !== "webp"} value={settings.format === "jpeg" || settings.format === "webp" ? settings.quality : "automatic"} onChange={(event) => update("quality", event.target.value as Quality)}>{settings.format === "jpeg" || settings.format === "webp" ? <><option value="low">Low</option><option value="medium">Medium</option><option value="high">High (Recommended)</option><option value="very-high">Very High</option><option value="maximum">Maximum</option></> : <option value="automatic">Automatic</option>}</select></label><label>Resolution (PPI)<select value={settings.ppi} onChange={(event) => update("ppi", Number(event.target.value))}>{ppiOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div>
